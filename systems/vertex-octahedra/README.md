# Vertex Octahedra

![Vertex Octahedra](00_thumb.png)

## Description

Platonic Octahedra solids aggregation, joined at the vertices. From the Wasp Example Files collection.

## Information

| Field | Value |
|---|---|
| Slug | `vertex-octahedra` |
| Author | [Andrea Rossi](https://thecomputationalhive.com/) |
| License | GPL-3.0 |
| Tags | `geometry` `platonic` `example` |
| Software | v0.6... |
| Units | Millimeters |
| Parts | 1 |
| Rules | 6 |

## Files

- [aggregation.json](aggregation.json)
- [meta.json](meta.json)

---

This README was generated automatically from `meta.json`.
